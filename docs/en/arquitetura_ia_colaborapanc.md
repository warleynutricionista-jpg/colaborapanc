...
- Function exposed: `identificar_com_multiplos_providedores(...)`.
...
- `PredicaoIA`: structured result of the inference.
- `Expert Validation`: final human decision.
- `HistoryValidation`: auditable events of the cycle.
...
4. Defines global score and operational range (`high`, `medium`, `low`).
...
- Textual justification by prediction (`justification`).
- Components of the territorial score on the dashboard (`components`).
- Event history in `HistoricoValidacao`.
