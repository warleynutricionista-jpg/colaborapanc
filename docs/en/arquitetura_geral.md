...
- **Signals**: `mapping/signals.py` (gamification scoring and progression).
- **Commands**: `mapping/management/commands/`.

## Database
- Engine configured: `django.contrib.gis.db.backends.postgis`.
- Geographic central entity: `PontoPANC` with `PointField` (`localizacao`) and lat/lon metadata.
...
4. Reviewer validates/rejects and generates `Expert Validation`.
...
- The project focuses many responsibilities on the `mapping` app.
