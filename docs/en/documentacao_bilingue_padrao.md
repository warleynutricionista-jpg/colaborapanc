...
### Accepted formats
1. **Bilingual single file (recommended for short/medium documents)**
   - Minimum structure:
     - `## Portuguese (PT-BR)`
     - `## English (EN)`
...
- When there is partial status marking, use:
  - `[PARTIALLY IMPLEMENTED]` in PT-BR
...
- Same technical depth in both versions.- When partial implementation status is needed, use:
  - `[PARTIALLY IMPLEMENTED]` in PT-BR
  - `[PARTIALLY IMPLEMENTED]` in EN
