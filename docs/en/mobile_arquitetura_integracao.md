...
1. **Inconsistent API Agreement**
   - Endpoint mobile pointed to `cadastrar_ponto_api/` (not found in current URLs).
   - `offlineService` used `api.get()`/`api.post()` without there being a real HTTP client.
   - Divergent `location` formats (`[lng,lat]` in the app vs GeoJSON in part of the API).
...
3. **Navigation with broken routes**
   - Home called `Alerts`/`Profile`, but stack exposed other routes.
...
- `mobile/src/config/api.js`
  ...
  - corrects registration route to `/api/pontos/`.
...
### 3.4 Focus species + AI + drone (structural basis)- `mobile/src/services/speciesFocusService.js`
...
1. Validate mobile authentication with backend (token/session) and establish a single contract.
...
4. Create drone missions/batches screen using `droneMissionService`.
...
- Do not change canonical field names (`popular_name`, `scientific_name`, `status_validacao`, `localizacao`).
...
### Online tests
- Login and load the map (`/api/pontos/`).
- Point registration with geolocation and image.
- Reference synchronization (`/api/offline-sync/`).
...
- Check history recording with `model_version`, confidence and revision status.
...
- `mobile/src/screens/PlantasOfflineScreen.js`- starts displaying status of the targeted offline base and generates base after selected download
