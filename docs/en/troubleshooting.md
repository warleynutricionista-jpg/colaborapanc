...
## Backend does not start: `DJANGO_SECRET_KEY not defined`
...
- On a physical device, do not use `localhost` on the host machine.

## Permission denied in scientific review
- User must be a superuser or member of the `Reviewer` group.