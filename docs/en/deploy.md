...
- Static files and media strategy (`collectstatic`, storage).

## Base step by step (backend)
1. Provision Linux environment with Python and GDAL.
2. Configure PostGIS database.
3. Set production `.env` (`DEBUG=False`).
4. Install dependencies and run migrations.
5. Run `collectstatic`.
6. Upload application with WSGI (`config/wsgi.py`) via Gunicorn/uWSGI and reverse proxy.

## Mobile
- Build Expo according to the environment (preview/prod), with `EXPO_PUBLIC_API_URL` pointing to the public backend.
